...
namespace JsonMigrator
{
    public static class JsonMigrator
    {
        ...
        public static JToken Migrate<T>(JToken token)
        {
            var type = typeof(T);
            ...
            var migrationMethods = type.GetMethods(BindingFlags.NonPublic | BindingFlags.Static | BindingFlags.Public | BindingFlags.Instance).Where(m => m.GetCustomAttribute(_migrationAttirbuteType) != null).ToList();
